...
2. **Create translated versions** for each specified target language:
   ...
   - Not translate english words `skills` `rules` in other language versions
...
3. **Update language navigation links** at the top of each document:
   ...
   - Ensure all documents include complete, consistent navigation; navigation to itself shows plain text instead of a link, use · to separate each item.
